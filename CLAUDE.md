...
- v0.14 — Home-redesign. De grote masthead is vervangen door een slanke sticky topbar (`.home-topbar`: locatie-eyebrow + titel + theme-toggle), wat verticale ruimte bespaart en de zoekbalk hoger brengt. De zeven categorie-secties zijn nu een inklapbare accordeon: standaard staat alles open, elke sectie klapt afzonderlijk dicht via de `.section-head`-knop, en de ingeklapte staat wordt onthouden in `localStorage` via het `Collapsed`-util-object (`has` / `toggle` / `setAll` / `clear`, key `praktijktools.collapsed`). Daarnaast een lijst/kaarten-weergaveschakelaar via het `View`-util-object (key `praktijktools.homeview`): lijst-modus rendert compacte regels (`toolRow` — naam + bronlabel + chevron, teal-stip bij featured, ster bij favoriet), kaarten-modus de bestaande `toolCard`-grid. De catjump-chips klappen een ingeklapte categorie automatisch uit bij het springen. App-handlers: `App.setView`, `App.toggleCategory`, `App.toggleAllCategories`. Geen nieuwe tools — totaal 37.
